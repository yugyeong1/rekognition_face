...
class RekognitionCompare(Resource):
    def post(self) :
        # 1. 클라이언트가 보낸 데이터를 받는다.fl
        ...
        try :
            client.upload_fileobj(sourceFile,
                                    ...
                                    ExtraArgs = {'ACL':'public-read', 'ContentType' : sourceFile.content_type } )

            client.upload_fileobj(targetFile,
                                    Config.S3_BUCKET,
                                    "target" + new_file_name,
                                    ExtraArgs = {'ACL':'public-read', 'ContentType' : targetFile.content_type } )
        
        except Exception as e:
            return {'error' : str(e)}, 500


        # 3. S3에 저장된 사진을 compare_faces 한다
        #    (AWS Rekognition 이용)
        client = boto3.client('rekognition',
                    'ap-northeast-2',
                    aws_access_key_id=Config.ACCESS_KEY,
        ...
        for faceMatch in response['FaceMatches']:
            ...
            result_list.append('The face at ')
